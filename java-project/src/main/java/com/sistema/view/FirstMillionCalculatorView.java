...
/**
 * Janela para simulacao do tempo ate o primeiro milhao.
 */
public class FirstMillionCalculatorView extends JDialog {

    /**
     * Valor alvo usado na simulacao financeira.
     */
    private static final BigDecimal META = BigDecimal.valueOf(1_000_000);
    ...
    /**
     * Cria uma nova instancia de FirstMillionCalculatorView.
     *
    ...
    private JPanel criarRodape() {
        ...
        JButton btnCalcular = criarBotao("Calcular", COR_VERDE);

        btnFechar.addActionListener(e -> dispose());
        btnLimpar.addActionListener(e -> limparCampos());
        btnCalcular.addActionListener(e -> calcularPrimeiroMilhao());
     ...
     */
    private void calcularPrimeiroMilhao() {
        try {
            BigDecimal patrimonio = parseMoeda(txtCapitalInicial.getText());
            BigDecimal aporteMensal = parseMoeda(txtAporteMensal.getText());
            BigDecimal taxaMensal = parsePercentual(txtTaxaJuros.getText());

            if (patrimonio.compareTo(BigDecimal.ZERO) < 0
            ...
            while (patrimonio.compareTo(META) < 0 && meses < limiteMeses) {
                patrimonio = patrimonio.multiply(fator, mc).add(aporteMensal, mc);
                meses++;
                evolucao.add(patrimonio);
            }
